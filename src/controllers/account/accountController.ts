...
// fonction utilitaire pour filtrer les données sensibles de l'utilisateur
const sanitizeUser = (user: any) => {
  const userObj = typeof user.toObject === 'function' ? user.toObject() : user;
  const { _id, firstName, lastName, email, activationDate, lastLogin, roles } = userObj;
  return { _id, firstName, lastName, email, activationDate, lastLogin, roles };
};
...
const createUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return responseWrapper.sendSuccess(201, 'Utilisateur créé avec succès.', sanitizeUser(user));
  } catch (error) {
...
const getUsers = async (_req: Request, res: Response) => {
  ...
  try {
    ...
    // On filtre chaque utilisateur
    const safeUsers = users.map(user => sanitizeUser(user));
    return responseWrapper.sendSuccess(200, 'Utilisateurs récupérés avec succès.', safeUsers);
  } catch (error) {
    return responseWrapper.sendError(500, 'INTERNAL_SERVER_ERROR', 'Erreur serveur.', error);
  }
};
...
const getUserById = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return responseWrapper.sendSuccess(200, 'Utilisateur récupéré avec succès.', sanitizeUser(user));
  } catch (error) {
    return responseWrapper.sendError(500, 'INTERNAL_SERVER_ERROR', 'Erreur serveur.', error);
  }
};
...
const updateUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return responseWrapper.sendSuccess(200, 'Utilisateur mis à jour avec succès.', sanitizeUser(user));
  } catch (error) {
    return responseWrapper.sendError(500, 'INTERNAL_SERVER_ERROR', 'Erreur serveur.', error);
  }
};
